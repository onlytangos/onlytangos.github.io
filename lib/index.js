...
function printNext7d(titulo){
	...
	for (let j=0;j<data.length;j++){
		...
		if (campos[9]!=''&&campos[9]!=undefined){
			let href='img/'+campos[9];			
...
function printEurope(ftitulo,fpais){
	...
	for (let j=0;j<data.length;j++){	
		let cp=data[j].split(';');
		let ini=new Date(cp[6]);
		let fin=new Date(cp[7]);
		if (!isNaN(ini)){
			ini=printDate(ini);
		}else{
			ini='';
		...
		linea=cp[0]+';'+cp[1]+';'+cp[2]+';'+cp[3]+';'+cp[4]+';'+cp[5]+';'+ini+';'+fin+';'+cp[8];
	...
	for (let j=0;j<dat1.length;j++){
		...
		let nombre=campos[3];
		let ciudad=campos[4];			
		let ini=new Date(campos[6]);
		let fin=new Date(campos[7]);
		let src=campos[8];
