...
var gx=window.matchMedia('(max-width:600px)');
...
function inicia(){
	...
	//gx.addEventListener('change',function(){mobil(gx)});
}
function mobil(gx){
	if (gx.matches){
		return true
...
function printNext7d(titulo){
	...
	if (!mobil(gx)){
		res+='<table class="tabla1">';	
	}else{
		res+='<table class="tabla2 grande">';
	}
	...
	for (let j=0;j<data.length-1;j++){
		let linea='';
		let [tipo,nombre,dir,fecha,ini,,fin,,,href,cont]=data[j].split(';');
